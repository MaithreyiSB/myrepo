...
#Input image path
test_img = cv2.imread("F:\Python Projects\Task1\quotedtext.png")

#pytesseract accepts RGB value, test image is converted to RGB from BGR
...
for x, c in enumerate(data.splitlines()):
    if x != 0:
        c = c.split()
        if len(c) == 12:
            #Initialising 4 variables for x coordinate, y coordinate, width, height
            x, y, w, h=int(c[6]), int(c[7]), int(c[8]), int(c[9])

            #creating boxes around characters
